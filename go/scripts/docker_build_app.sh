#!/bin/bash

#BASE_VER=0.2
#APP_VER=0.4
#
#cp -f ../Dockerfile ../Dockerfile-bkup
#cp -f ../Dockerfile-app ../Dockerfile
#
#docker build \
#    --build-arg ssh_pub_key="ssh-rsa  jenkins@a7131a1767c9" \
#    --build-arg BUILD_IMAGE=lyw1217/ojeommu-base:${BASE_VER} \
#    -t lyw1217/ojeommu:${APP_VER} "../"

#TAG="0.5"
TAG="0.6"

#docker build --tag lyw1217/ojeommu:${TAG} "../"
#docker buildx build --push --platform linux/arm/v7,linux/arm64/v8,linux/amd64 --tag lyw1217/ojeommu:${TAG} "../"
docker buildx build --push --platform linux/amd64 --tag lyw1217/ojeommu:${TAG} "../"